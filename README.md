...
````
Go to a directory, where you want to install OncoFEM. OncoFEM is downloaded from GitHub and an anaconda environment is 
set up, wherein the package `oncofem` will be installed. In the installation also global path variables will be 
installed and the terminal needs to be restarted.
````bash
chmod +x install_oncofem.sh
./install_oncofem.sh
````
...
SVMTK could not be installed on the macOS test environment. 
````bash
cd ..
