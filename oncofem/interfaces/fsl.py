...
class FSL:
    ...
    def run_stats(self, command: list):
        """
    ...
    def run_fast(self, command: list):
        """
        FAST 
    ...
    def run_fslhd(self, file: str):
        ...
        list = [entry.replace("\t\t", "\t") for entry in output.splitlines()][2:]
        list = [entry.split("\t") for entry in list]
        arguments = {entry[0]: entry[1] for entry in list}
        return arguments
