...
from dolfin import Constant, conditional, gt, le
...
class SimpleModel(BioChemModel):
    ...
    def return_prod_terms(self):
        ...
        hat_Ft_Fn_gain = Constant(0.0)
        hat_St_Fn_gain = Constant(0.0)
        if self.flag_proliferation:
            H1 = conditional(gt(cFn, self.cFn_min_growth), 1.0, 0.0)  # Enough nutrients
            H3 = conditional(gt(cFt, self.cFt_ms), 1.0, 0.0)  # mobile cancer cells above threshold
            # Translation of micrometastatic switch 
            H4 = conditional(gt(nSt, self.nSt_thres_lin_ms),
                             nSt * self.rhoStR * self.nu_St_proliferation, 
                             self.fac_nSt_lin_ms * cFt * self.nu_St_proliferation)

            fac_max_cFt = conditional(gt(1.0 - (cFt / self.cFt_max), 0.0), 1.0 - (cFt / self.cFt_max), 0.0)
            fac_max_nSt = conditional(gt(1.0 - (nSt / self.nSt_max), 0.0), 1.0 - (nSt / self.nSt_max), 0.0) 
        ...
        hat_St_Fn_loss = Constant(0.0)
        hat_Ft_Fn_loss = Constant(0.0)
        hat_Sh_Fn_loss = Constant(0.0)
        hat_Sn_Fn_gain = Constant(0.0)
        if self.flag_necrosis:
            H2 = conditional(le(cFn, self.cFn_min_necrosis), 1.0, 0.0)  # Start of Necrosis
