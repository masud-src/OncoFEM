...
class GBMRatioCalibration(ProcessModel):
    def __init__(self):
        ...
        self.nu_Ft_proliferation = 6.64e9
        # nutrients
    ...
    def get_alternative_output(self):
        u, p, nSh, nSt, nSn, cFt, cFn = self.prim_vars
        nS = nSh + nSt + nSn
        nF = 1.0 - nS
        ...
        # No Nutrients left
        cond_5 = ufl.gt(cFn, 0.05)
        H5 = ufl.conditional(cond_5, df.Constant(1.0), df.Constant(0.0))

        # Proliferation of mobile cancer cells
        hat_Ft_Fn_gain = cFt * df.Constant(3.5e9) * (1.0 - cFt / df.Constant(self.cFt_max)) * self.dt / 3600

        # Proliferation of tumour
        hat_St_Fn_gain = H1 * H5 * df.Constant(1.0e-4) * (1.0 - nSt / df.Constant(0.75))

        # Metabolism
        hat_cFn = - H5 * nSt * df.Constant(0.05e-4)

        # Necrosis
        hat_Sn_gain = H4 * (1.0 - cFn) * df.Constant(3e-5) * (1.0 - nSn / df.Constant(0.75))  

        hat_nSh = - (1.0 - H4) * hat_St_Fn_gain 
        hat_nSt = (1.0 - H4) * hat_St_Fn_gain - hat_Sn_gain
        hat_nSn = hat_Sn_gain
        hat_cFt = hat_Ft_Fn_gain
    ...
    @staticmethod
    def w(x, w_0, w_delta, x_0, a):
        return w_delta / (1 + np.exp(-a * (np.array(x) - np.array(x_0)))) + w_0
