...
# MRI PRE-PROCESSING
#
# From that the mri entity can be set up by giving the particular state. All measurements of that state will  be load
# and the most important structural MRIs (t1, t1gd, t2, flair, seg) are set. Furthermore, the affine of the first image
# is safed as a general quantity in the mri entity. First thing that is need to be evaluated, is the tumors spatial
# distribution and composition. Therefore, in a next step the tumor entity is set up. Since, this is already given in
# the input state, this result can be set directly. To identify the particular compartment of the tumor (active,
# necrotic, edema), the command 'set_compartment_masks()' is executed.
...
# the tumor. Both approaches are discussed in tutorial 'tut_02_mri'. The user can further chose which inputs shall be
# used and the segmentation can be run with the 'run()' command. Since this step can take several time, the already
# performed output files can be used from the data folder.
run_wms = False
if run_wms:
...
# folder. Note, that the mapping is performed onto the generated file in the data folder. The mesh generator can also
# generate different meshes at the same resolution and it is only ensured that both match if they were generated
# together. Various settings can also be made, which are discussed in the associated tutorial.
...
# To show the derivation of the mathematical construct of that model as well as a blueprint to implement own models, see
...
# problem. Since, the TwoPhaseModel is a continuum-mechanical model that is solved within the finite element method,
...
# arbitrary resolved components into the fluid phase, e.g. glucose, oxygen, growth factors, etc. This is usefull,
