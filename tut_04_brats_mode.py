...
tutorial, a convenient function is presented which generates the necessary input data from a BraTS-like segmentation 
file. In the here preserved segmentation file, following classes are present:
...
- active tumour part goes from a maximum at the necrotic border to minimum at the outer rim to the edema
- necrotic is constant in necrotic area (this is assumed due to numerical issues)

In order to map quantities onto a mesh, first thing that needs to be done is the creation of that mesh, like in the last
...
When the tutorial is finished, the user can continue with the simulation analogously to previous tutorials.
"""
...
#
study = of.structure.Study("tut_04")
...
#
fmap = of.FieldMapGenerator(study.der_dir + "const_wm" + os.sep)
...
# interpolation and the respective method is set (linear, nearest). More information about the methods can be found in
# the documentation of scipy.interpolate.griddata. Two different mappings need to be done. First the distribution of
# the tumour compartments need to be done in order to know the extend of the disease and second, material parameters
# need to be chosen. Therefore, the "set_mixed_masks" command merges information from the tumour and structural 
# segmentation according to one of the two approaches. First, "const_wm" assumes the tumour area to act like white 
# matter, "mean_averaged_values" merges a first tumour class to the white matter, a second to the gray matter, and a 
# third to the csf. Again, the affine is set to map everything to the same space.
#
...
#
# In the next part, pre-mapped partial continuous tumour classes are clustered, that can be used as an input for the
# mean averaged value approach.
#
...
fmap.work_dir = study.der_dir + "mean_averaged_value" + os.sep
